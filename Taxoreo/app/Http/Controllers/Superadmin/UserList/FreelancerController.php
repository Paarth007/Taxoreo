...
{
    private $url="superadmin/user-list/freelancer";
    private $document_url="";

    public function index(){
        return view('Superadmin.UserList.Freelancer.Index')->with(['url'=>$this->url]);
    }

    public function create(Request $request){
        return view('superadmin.UserList.Freelancer.Form')->with(['id'=>NULL,'url'=>$this->url,'document'=>NULL]);
    }

    public function edit(Request $request,$id){
        $document=DB::table('master_documents')->where('id',$id)->first();
        return view('superadmin.UserList.Freelancer.Form')->with(['id'=>$id,'url'=>$this->url,'document'=>$document]);
    }
    ...
    public function store(Request $request)
    {
        ...
        if($insertGetId > 0){
            $request->session()->flash('alert-success', 'Freelancer Details Added Successfully !!! ');
            if($request->button_type && $request->button_type=="SAVE_AND_ADD_ANOTHER"){
                return redirect($this->url."/create");
            }
            return redirect($this->url);
        }else{
            $request->session()->flash('alert-warning', 'Freelancer Details Not Added !!! ');
            return redirect($this->url."/create");
        }
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        if($is_updated > 0){
            $request->session()->flash('alert-success', 'Freelancer Details Updated Successfully !!! ');
            return redirect($this->url);
        }else{
            $request->session()->flash('alert-warning', 'Freelancer Details Not Added !!! ');
    ...
    public function show(Request $request)
    {
        ...
        $data=$query->where('u.user_type','FREELANCER')
