...
{
    private $url="superadmin/user-list/ca";
    private $document_url="";

    public function index(){
        return view('Superadmin.UserList.Ca.Index')->with(['url'=>$this->url]);
    }

    public function create(Request $request){
        return view('superadmin.UserList.Ca.Form')->with(['id'=>NULL,'url'=>$this->url,'document'=>NULL]);
    }

    public function edit(Request $request,$id){
        $document=DB::table('master_documents')->where('id',$id)->first();
        return view('superadmin.UserList.Ca.Form')->with(['id'=>$id,'url'=>$this->url,'document'=>$document]);
    }
    ...
    public function store(Request $request)
    {
        ...
        if($insertGetId > 0){
            $request->session()->flash('alert-success', 'CA Details Added Successfully !!! ');
            if($request->button_type && $request->button_type=="SAVE_AND_ADD_ANOTHER"){
                return redirect($this->url."/create");
            }
            return redirect($this->url);
        }else{
            $request->session()->flash('alert-warning', 'CA Details Not Added !!! ');
            return redirect($this->url."/create");
        }
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        if($is_updated > 0){
            $request->session()->flash('alert-success', 'CA Details Updated Successfully !!! ');
            return redirect($this->url);
        }else{
            $request->session()->flash('alert-warning', 'CA Details Not Added !!! ');
    ...
    {
        $is_destroyed=DB::table('users')->where('user_type','CA')->where('id',$id)->update(['is_active'=>0,'updated_at'=>NOW()]);
        if($is_destroyed > 0){
            $request->session()->flash('alert-success', 'CA deleted !!! ');
        }else{
            $request->session()->flash('alert-warning', 'CA not deleted !!! ');
        }
    ...
    public function show(Request $request)
    {
        ...
        $data=$query->where('u.user_type','CA')
