...
{
    private $url="superadmin/master/document";
    private $document_url="download_document";

    public function index(){
        return view('Superadmin.Master.Document.Index')->with(['url'=>$this->url]);
    }

    public function create(Request $request){
        return view('superadmin.Master.Document.Form')->with(['id'=>NULL,'url'=>$this->url,'document'=>NULL]);
    }

    public function edit(Request $request,$id){
        $document=DB::table('master_documents')->where('id',$id)->first();
        return view('superadmin.Master.Document.Form')->with(['id'=>$id,'url'=>$this->url,'document'=>$document]);
    }
    ...
    public function store(Request $request)
    {
        ...
        if($insertGetId > 0){
            ...
            $request->session()->flash('alert-warning', 'Document Not Added !!! ');
            return redirect($this->url."/create");
        }
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        if($is_updated > 0){
            $request->session()->flash('alert-success', 'Document Updated Successfully !!! ');
            return redirect($this->url);
        }else{
            $request->session()->flash('alert-warning', 'Document Not Added !!! ');
    ...
    {
        $is_destroyed=DB::table('master_documents')->where('id',$id)->update(['is_active'=>0,'updated_at'=>NOW()]);
        if($is_destroyed > 0){
            $request->session()->flash('alert-success', 'Documents deleted !!! ');
        }else{
            $request->session()->flash('alert-warning', 'Documents not deleted !!! ');
        }
    ...
    {
        $query = DB::table('master_documents');
